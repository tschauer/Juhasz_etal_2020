...
```{r}
library(gdata)
dataset <- read.delim("raw_data/FigureS5I.txt", stringsAsFactors = FALSE)

colnames(dataset) <- gsub("NHEJ.","Outcome", colnames(dataset))

dataset$genotype <- factor(gsub(" ","",dataset$genotype), levels = c("mCherry", "mCherry-ALC1"))
...
for(gid in levels(dataset$genotype)){
        dataset$Offset2[dataset$genotype == gid] <- mean(dataset$NormOutcome[dataset$genotype == "mCherry"])
...
ggplot(dataset, aes(x=genotype, y=Outcome)) + 
        ...
        scale_color_manual(values=c("#000000","#808000","#808080","#000080"))
...
dataplot$genotype <- factor(dataplot$genotype, levels = c("mCherry", "mCherry-ALC1"))
...
ggplot(dataplot, aes(x=genotype, y=NormOutcome2, fill = genotype)) + 
        ...
        ylim(0, 1.8) +
        scale_fill_manual(values=c("#000000","#808000","#808080","#000080"))



library(Cairo)

cairo_pdf("FigureS5I_v1.pdf", width = 5, height = 4, family = "Arial")
...
cairo_pdf("FigureS5I_v2.pdf", width = 5, height = 4, family = "Arial")
...
ggplot(dataplot, aes(x=genotype, y=NormOutcome2, fill = genotype)) + 
        ...
        xlab(label = "NHEJ assay") +
        ylim(0, 1.8) +
        scale_fill_manual(values=c("#00000080","#80800080","#80808080","#00008080")) +
...
cairo_pdf("FigureS5I_v3.pdf", width = 5, height = 4, family = "Arial")
...
write.table(output, file = "FigureS5I_Stats_Ref_mCherry.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```

```{r, fig.height=5, fig.width=18}
# re-fit with mCherry-ALC1 reference
dataset$genotype <- relevel(dataset$genotype, ref = "mCherry-ALC1")
...
write.table(output, file = "FigureS5I_Stats_Ref_mCherry-ALC1.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
