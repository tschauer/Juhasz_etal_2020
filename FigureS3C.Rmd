...
```{r}
dataset <- read.delim("raw_data/FigureS3C.txt", stringsAsFactors = FALSE)
...
cairo_pdf("FigureS3C.pdf", width = 5, height = 4, family = "Arial")
...
write.table(output, file = "FigureS3C_Stats_Ref_WT.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
...
write.table(output, file = "FigureS3C_Stats_Ref_ALC1KO.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
...
for(g in seq_along(GSID_levels)){
        ...
        fit_group <- lm(NormCounts2 ~ poly(H2O2,3)*GSID, data = dataset)
        ...
        if(g == 1){
                ...
                outtmp <- outtmp[grep("H2O2", rownames(outtmp)),]
...
write.table(output, file = "FigureS3C_Stats_New_All.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
