...
```{r}
dataset <- read.csv("raw_data/20200803 Rad51.csv", header = F)
...
ggplot(dataset, aes(x=group, y=Outcome)) + 
        ...
        scale_shape_manual(values=14:20) +
...
cairo_pdf("20200803 Rad51.pdf", width = 6, height = 5, family = "Arial")
...
write.table(output, file = "20200803 Rad51_Stats_Ref_WT_NT.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
...
write.table(output, file = "20200803 Rad51_Stats_Ref_PARP1KO_NT.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
...
write.table(output, file = "20200803 Rad51_Stats_Ref_ALC1KO_NT.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
