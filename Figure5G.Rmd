...
```{r, fig.height=5, fig.width=18}
# re-fit with WT siBRCA1 reference
...
```{r, fig.height=5, fig.width=18}
# re-fit with YFP-ALC1 siBRCA1 reference
dataset$genotype <- relevel(dataset$genotype, ref = "YFP-ALC1")
dataset$siRNA <- relevel(dataset$siRNA, ref = "siBRCA1")
...
write.table(output, file = "Figure5G_Stats_Ref_YFP-ALC1_siBRCA1.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
...
write.table(output, file = "Figure5G_Stats_New_All.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
