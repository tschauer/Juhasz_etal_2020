...
pdf("Figure5G.pdf", width = 8, height = 4)

ggplot(dataset, aes(x=Olaparib, y=NormCounts2)) + 
        theme_bw() +
        theme(panel.grid=element_blank(), text = element_text(size=14)) +
        geom_point(aes(colour = BRCA)) +
        geom_smooth(method=lm, formula = y ~ poly(x,3), se=TRUE, aes(colour = BRCA), fill='#CCCCCC') +
        facet_grid(. ~ Alc1) +
        xlab(label = "Olaparib (log10 nM)") +
...
pdf("Figure5G.pdf", width = 5, height = 4)
...
write.table(output, file = "Figure5G_Stats_Ref_h_d11.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
...
write.table(output, file = "Figure5G_Stats_Ref_d11_d11.txt", quote = F, sep = "\t", row.names = T, col.names = NA)
```
